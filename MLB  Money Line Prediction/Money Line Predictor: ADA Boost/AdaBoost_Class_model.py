...
with open('MLBdata_2016_2020_Full.csv') as csv_file:
...
df1 = pd.read_csv("MLBData_2016_2020_Full.csv")
...
param_grid = {'n_estimators': sp_randint(100, 1000), 'learning_rate': sp_uniform(0.001,0.3) }
...
model = RandomizedSearchCV(model, param_distributions = param_grid, n_iter = 100, scoring = 'recall', cv=cv, return_train_score = True, refit = True).fit(X_train, Y_train)
